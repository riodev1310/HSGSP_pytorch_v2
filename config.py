...
class Config:
    ...
    validation_split: float = 0.1  # Giảm để có train set lớn hơn (45k samples)
    ...
    default_epochs: int = 300  # Tăng để converge tốt hơn
    initial_lr: float = 0.05  # Cao hơn cho SGD start nhanh
    pruned_growth_lr: float = 1e-4
    min_lr: float = 1e-5
    momentum: float = 0.9  # for SGD
    optimizer: str = 'sgd'  # Thay adamw bằng sgd cho acc cao hơn trên CIFAR

    # Learning rate schedule
    lr_schedule: str = 'step'  # Step decay đơn giản và hiệu quả cho VGG
    lr_warmup_epochs: int = 5
    lr_decay_rate: float = 0.1  # Decay by 0.1
    lr_decay_steps: int = 80  # Decay every 80 epochs (e.g., at 80, 160, 240)
    ...
    # Regularization
    l2_regularization: float = 5e-4  # Tăng nhẹ so với 1e-4 cho CIFAR
    batch_norm_momentum: float = 0.99  # Tăng để smooth hơn

    dropout_rate: float = 0.2  # Giảm convolutional dropout
    ...
    use_cutout: bool = True  # Thêm Cutout để boost acc (implement in data pipeline)
    cutout_length: int = 16  # Kích thước cutout cho 32x32 images
